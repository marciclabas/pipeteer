from typing_extensions import TypeVar, Generic, Callable, Awaitable, get_type_hints
...
@dataclass
class Activity(Pipeline[A, B, Ctx], Generic[A, B, Ctx]):
...
def activity(name: str | None = None, *, reserve: timedelta | None = timedelta(minutes=1)):
